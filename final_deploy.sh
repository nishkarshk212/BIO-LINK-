...
# Final deployment script for Bio Guard Bot with latest code
SERVER_IP="140.245.240.202"
SERVER_USER="root"

echo "Starting deployment of latest Bio Guard Bot to $SERVER_IP..."
...
echo "✓ Local files verified (latest version with Open Here/Open in Private feature)"
...
echo "- Implemented callback handler for 'Open Here' button"
echo "- Updated /start command to handle /start settings parameter"
echo ""

# SSH into the server and set up the bot with embedded files
echo "Attempting to connect and deploy to server..."
echo "NOTE: If connection fails, the server may still be limiting connections due to MaxStartups."
echo "Wait at least 15-30 minutes between connection attempts."
echo ""

ssh -p 22 $SERVER_USER@$SERVER_IP << EOF
set -e

echo "Connected to server, starting setup with latest code..."
...
echo "Files transferred successfully (latest version)"
...
if [ ! -d "venv" ]; then
    ...
    echo "Virtual environment already exists, recreating to ensure clean state..."
    rm -rf venv
    python3 -m venv venv
...
echo "Service started with latest code"
...
if systemctl is-active --quiet bio-guard-bot.service; then
    ...
    echo "- Latest version deployed with Open Here/Open in Private feature"
...
if [ \$? -eq 0 ]; then
    ...
    echo "Bio Guard Bot with the latest code is now running on the server."
    echo ""
    echo "New Feature Summary:"
    ...
    echo "- Connection may have been reset due to MaxStartups limit on the server"
    echo "- Wait 15-30 minutes before trying again"
    echo "- Contact server administrator to check SSH configuration"
