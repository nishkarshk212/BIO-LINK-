...
if command -v base64 >/dev/null 2>&1; then
    # Check if on macOS (Darwin) or Linux
    if [[ "$OSTYPE" == "darwin"* ]]; then
        # macOS uses -b for line wrapping
        BIO_GUARD_BOT=$(base64 -b 0 bio_guard_bot.py)
        REQUIREMENTS=$(base64 -b 0 requirements.txt)
        ENV_FILE=$(base64 -b 0 .env)
    else
        # Linux uses -w for line wrapping
...
echo "Waiting 5 seconds before attempting connection..."
sleep 5
...
# Use SSH with options to prevent interactive prompts
ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o ConnectTimeout=30 -o BatchMode=yes root@140.245.240.202 << 'ENDSSH'
set -e

echo "Connected to server, starting deployment with latest code..."
...
cat > /tmp/bot_b64.txt << 'BOTEOF'
$(echo $BIO_GUARD_BOT)
BOTEOF
cat > /tmp/req_b64.txt << 'REQEOF'
$(echo $REQUIREMENTS)
REQEOF
cat > /tmp/env_b64.txt << 'ENVEOF'
$(echo $ENV_FILE)
ENVEOF

base64 -d /tmp/bot_b64.txt > /opt/bio_guard_bot/bio_guard_bot.py
base64 -d /tmp/req_b64.txt > /opt/bio_guard_bot/requirements.txt
base64 -d /tmp/env_b64.txt > /opt/bio_guard_bot/.env

# Clean up temporary files
rm /tmp/bot_b64.txt /tmp/req_b64.txt /tmp/env_b64.txt
...
if [ $? -eq 0 ]; then
    ...
    echo ""
    echo "✗ Error occurred during server deployment"
    echo ""
