...
# Redeployment script for latest Bio Guard Bot with all button fixes
...
echo "If connection fails, the server may still be limiting connections due to MaxStartups."
echo "Server response may take up to 30 seconds..."
echo ""

ssh -p 22 -o ConnectTimeout=30 -o ServerAliveInterval=10 $SERVER_USER@$SERVER_IP << EOF
set -e
...
if [ $? -eq 0 ]; then
    ...
    echo "- Wait at least 30-60 minutes before trying again to allow connection limits to reset"
    echo "- Contact server administrator to check SSH configuration (MaxStartups setting)"
    echo "- Verify the server has sufficient disk space and resources"
    echo "- Check if the server has any firewall rules blocking connections"
    echo ""
    echo "Before trying again, you may want to:"
    echo "1. Wait longer (connection limits may take time to reset)"
    echo "2. Try accessing the server through a web-based console if available"
