...
if command -v base64 >/dev/null 2>&1; then
    # On macOS, base64 -b 0 sets line length to 0 (no wrapping)
...
echo "Server response may take up to 60 seconds..."
echo ""

# Use SSH with options to prevent interactive prompts
ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o ConnectTimeout=60 -o BatchMode=yes -o LogLevel=ERROR root@140.245.240.202 << ENDSSH
set -e
...
if [ \$? -eq 0 ]; then
    ...
    echo "- The server may have connection limits that require waiting between attempts"
    exit 1
fi
